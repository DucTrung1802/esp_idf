...
#define DEVICE_ID "d74d08c9-2f69-473b-88cd-542be5af2c66"
#define DEVICE_TOKEN "WB2He02SZnTNIkjTWIA2j27sfXghHkC0"
...
void setup() {
  ...
  sprintf(topic_pub, "messages/%s/attributes", DEVICE_ID);
...
void Check_network() {
  bool res;
  Serial.println("in Check_network()");
POWER_ON:
  if (isInit(4)) {
    ESP_LOGW(TAG, "module connect physical success");
    Serial.println("module connect physical success");
  } else {
    ESP_LOGW(TAG, "module connect physical fail");
    Serial.println("module connect physical fail");
  }
FINE_NET:
  res = isRegistered(10);
  if (res) {
    ESP_LOGW(TAG, "Module registed OK");
    Serial.println("Module registed OK");
  }
  else {
    if (!isInit(3)) {
      ESP_LOGE(TAG, "Module registed FALSE");
      Serial.println("Module registed FALSE");
      goto POWER_ON;
    } else {
      goto FINE_NET;
    }
